...
class _VenueBookingScreenState extends State<VenueBookingScreen> {
  ...
  bool isButtonPressed(){
    return PeroidButtons.button1 || PeroidButtons.button2 || PeroidButtons.button3;
  }
...
Widget buildCard(DateTime dt, String chosenPeriod){
  ...
  return Card(
    ...
    shape: RoundedRectangleBorder(
      ...
      side: BorderSide(color: Colors.grey, width: 1), // add a border to the card
    ),
    child: ListTile(
  ...
  Widget build(BuildContext context) {
    ...
    body: SingleChildScrollView(child: Column(
      children: [
        ...
        child: ClipRRect(borderRadius: BorderRadius.all(Radius.circular(25)),
       ...
       SizedBox(height: 35,),
        ...
        Padding(padding: EdgeInsets.only(bottom: 10, top: 10),
            child: Text('Select a Date', style: GoogleFonts.almarai(fontWeight: FontWeight.bold, fontSize: 20),)),

        Container(padding: EdgeInsets.only(left: 30, right: 30, bottom: 40),
       ...
       // Container(padding: EdgeInsets.symmetric(vertical: 30), child: Center(child: Text('first, pick a date'),)),
        ...
        Padding(padding: EdgeInsets.only(bottom: 10, top: 10),
            child: Text('Select a Time', style: GoogleFonts.almarai(fontWeight: FontWeight.bold, fontSize: 20),)),

       PeroidButtons(refreshBookingScreen: this.refreshScreen, selectedDate: _dateTime, chosenVenue: routeArgs['venueObject'] as VenueItem,),

        Container(padding: EdgeInsets.only(top: 30),
            child: ElevatedButton(onPressed: !isButtonPressed()? null: (){
